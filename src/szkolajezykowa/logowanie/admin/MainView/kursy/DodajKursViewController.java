 ...
 * @author BlackHawk
 */
public class DodajKursViewController implements Initializable {
    ...
    @FXML
    private void dodajKurs(ActionEvent event) throws IOException, SQLException {
        ...
        if (sprawdzenie) {
            ...
            String query = "{CALL KRZYSZTOF.dodaj_kurs(?,?,?,?,?,?)}";
            CallableStatement cst = con.prepareCall(query);

            cst.setString(1, fNazwaKursu.getText());
            cst.setInt(2, Integer.parseInt(fIloscGodzin.getText()));
            cst.setString(3, cbPoziom.getSelectionModel().getSelectedItem().toString());
            cst.setString(4, cbJezyk.getSelectionModel().getSelectedItem().toString());
            cst.setInt(5, cbNrSali.getSelectionModel().getSelectedItem().getId_sali());
            cst.setInt(6, cbProwadzacy.getSelectionModel().getSelectedItem().getId_wykladowcy());
            ...
            tray.setMessage("Dodawanie kursu zakończyło się sukcesem!");
        ...
        if (!sprawdzenie) {
            powiadomMnie.play();
            tray.setTitle("Błąd!");
            tray.setMessage("Dodawanie kursu nie powiodło się. \n"
