...
import szkolajezykowa.logowanie.uzytkownik.MainView.zaliczenia.ZaliczeniaController;
 ...
 * @author BlackHawk
 */
public class EdytujZaliczenieViewController implements Initializable {
    ...
    @FXML
    private JFXButton zapiszZaliczenieBtn;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            ...
            PreparedStatement ps = con.prepareStatement("SELECT distinct kurs.nazwa_kursu,kurs.jezyk,kurs.poziom,kurs.ilosc_godzin,kurs.id_kursu FROM kurs,wykladowcy WHERE kurs.id_wykladowcy =" + LoginViewController.r);
            ...
            Logger.getLogger(EdytujZaliczenieViewController.class.getName()).log(Level.SEVERE, null, ex);
        }
...
"FROM kursanci,kurskursanci,kurs WHERE kurs.id_wykladowcy="+ LoginViewController.r+ " AND kurs.id_kursu=" + idKursu + " AND kurs.id_kursu=kurskursanci.id_kursu AND kurskursanci.id_kursanta=kursanci.id_kursanta");
                        ResultSet rs = ps.executeQuery();                        
                        while (rs.next()) {
                        ...
                        Logger.getLogger(DodajZaliczenieViewController.class.getName()).log(Level.SEVERE, null, ex);
                    }
        ...
        try {
            Connection con = Polaczenie.ConnectDB();
            PreparedStatement ps = con.prepareStatement("Select data_zal,typ,wynik FROM zaliczenia WHERE id_zaliczenia="+ZaliczeniaViewController.idZaliczenia);
            ResultSet rs = ps.executeQuery();
            rs.next();
            
            fPunkty.setText(rs.getString("wynik"));
            cbTyp.setValue(rs.getString("typ"));
            
            for (WykladowcaKursyDetails2 wkd : cbKurs.getItems()){
                if (wkd.getId_kursu() == ZaliczeniaViewController.idKursu){
                    cbKurs.setValue(wkd);
                }
            }
            
            for (KursanciDetails kd : cbKursant.getItems()){
                if (kd.getId_kursanta() == ZaliczeniaViewController.idKursanta){
                    cbKursant.setValue(kd);
                }
            }
            
            String data = rs.getString("data_zal");
            LocalDate ld = LocalDate.parse(data);
            ld.format(DateTimeFormatter.ISO_DATE);
            dtData.setValue(ld);
            //dtData.getEditor().setText(rs.getString("data_zal"));
    ...
    @FXML
    private void zapiszZaliczenie(ActionEvent event) throws IOException, SQLException {
        Connection con = Polaczenie.ConnectDB();
        boolean add = true;
        ...
        if (add){
            String query = "{CALL KRZYSZTOF.update_zaliczenie(?,?,?,?,?,?)}";
            CallableStatement cst = con.prepareCall(query);
            cst.setInt(1, ZaliczeniaViewController.idZaliczenia);
            cst.setString(2, dtData.getValue().toString());
            cst.setInt(3, Integer.parseInt(fPunkty.getText()));
            cst.setString(4, (String)cbTyp.getSelectionModel().getSelectedItem().toString());
            cst.setInt(5, cbKurs.getSelectionModel().getSelectedItem().getId_kursu());
            cst.setInt(6, cbKursant.getSelectionModel().getSelectedItem().getId_kursanta());
            cst.execute();
            cst.close();
            con.close();
            ...
            tray.setMessage("Zaliczenie zostało zaktualizowane");
            tray.setNotificationType(NotificationType.SUCCESS);
            tray.showAndDismiss(Duration.millis(1500));
        
        
            AnchorPane dodaj = FXMLLoader.load(getClass().getResource("ZaliczeniaView.fxml"));
            AnchorPane home_parent2 = (AnchorPane) zapiszZaliczenieBtn.getParent().getParent().getParent();
        ...
        if (!add){
            ...
            TrayNotification tray = new TrayNotification();
            tray.setTitle("Błąd!");
            tray.setMessage("Zaliczenie nie zostało zaktualizowane");
